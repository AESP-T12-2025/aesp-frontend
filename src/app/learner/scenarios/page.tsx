  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        setError(null);
        // GỌI API THẬT
        const data = await scenarioService.getScenarios(topicId || undefined);
        setScenarios(data);
      } catch (err: any) {
      ...
      {scenarios.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {scenarios.map((scenario: any) => (
            <div key={scenario.id} className="border p-6 rounded-xl shadow-sm bg-white">
              <h3 className="font-bold text-lg mb-2">{scenario.title}</h3>
              <p className="text-gray-600 text-sm mb-4">{scenario.description}</p>
              <Link 
                href={`/learner/practice/${scenario.id}`}
